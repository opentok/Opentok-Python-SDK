...
import hashlib
import httpx  # create_session, archiving
...
from .opentok import Roles, MediaModes, ArchiveModes, valid_archive_resolutions, logger
...
class ClientAsync(object):
    ...
    def __init__(
        ...
        self._app_version = __version__ if app_version is None else app_version
        self._user_agent = (
    ...
    def generate_token(
            ...
            expire_time=None,
            data=None,
            initial_layout_class_list=None,
    ):
        """
        ...
        if initial_layout_class_list is None:
            ...
            if isinstance(expire_time, datetime):
                ...
                try:
                    ...
                    raise OpenTokException(
                        "Cannot generate token, invalid expire time {0}".format(
                            expire_time
                        )
                    )
        else:
        ...
        if not text_type(session_id):
            raise OpenTokException(
                "Cannot generate token, session_id was not valid {0}".format(
                    session_id
                )
            )
        if not isinstance(role, Roles):
            raise OpenTokException(
                "Cannot generate token, {0} is not a valid role".format(role)
            )
        now = int(time.time())
        if expire_time < now:
            raise OpenTokException(
                "Cannot generate token, expire_time is not in the future {0}".format(
                    expire_time
                )
            )
        if expire_time > now + (60 * 60 * 24 * 30):  # 30 days
            raise OpenTokException(
                "Cannot generate token, expire_time is not in the next 30 days {0}".format(
                    expire_time
                )
            )
        ...
        try:
            ...
            parts = decoded_session_id.decode("utf-8").split("~")
        except Exception:
            raise OpenTokException(
                "Cannot generate token, the session_id {0} was not valid".format(
                    session_id
                )
            )
        if self.api_key not in parts:
            raise OpenTokException(
                "Cannot generate token, the session_id {0} does not belong to the api_key {1}".format(
                    session_id,
                    self.api_key
                )
            )
    ...
    async def create_session(
        ...
        if not isinstance(media_mode, MediaModes):
            raise OpenTokException(
                "Cannot create session, {0} is not a valid media mode".format(
                    media_mode
                )
            )
        if not isinstance(archive_mode, ArchiveModes):
            raise OpenTokException(
                "Cannot create session, {0} is not a valid archive mode".format(
                    archive_mode
                )
            )
        ...
        options["p2p.preference"] = media_mode.value
        options["archiveMode"] = archive_mode.value
        if archive_name is not None:
        ...
        if location:
            ...
            try:
                ...
                raise OpenTokException(
                    "Cannot create session. Location must be either None or a valid IPv4 address {0}".format(location)
                )
    ...
    async def start_archive(
        ...
        if not isinstance(output_mode, OutputModes):
            raise OpenTokException(
                "Cannot start archive, {0} is not a valid output mode".format(
                    output_mode
                )
            )
    ...
    async def stop_archive(self, archive_id):
        """
    ...
    async def delete_archive(self, archive_id):
        """
    ...
    async def get_archive(self, archive_id):
    ...
    async def get_archives(self, offset=None, count=None, session_id=None):
        """Returns an ArchiveList, which is an array of archives that are completed and in-progress,
        for your API key.

        :param offset: offset Optional (int). The index offset of the first archive. 0 is offset
        ...
        :param count: count Optional (int). The number of archives to be returned. The maximum
          number of archives returned is 1000.
        :param session_id: session_id Optional (string). Used to list archives for a specific session ID.
    ...
    async def remove_archive_stream(
    ...
    async def send_signal(self, session_id, payload, connection_id=None):
        """
    ...
    async def signal(self, session_id, payload, connection_id=None):
        warnings.warn(
            "opentok.signal is deprecated (use opentok.send_signal instead).",
            DeprecationWarning,
            stacklevel=2,
        )
        return await self.send_signal(session_id, payload, connection_id)

    async def get_stream(self, session_id, stream_id):
        """
    ...
    async def list_streams(self, session_id):
        """
    ...
    async def force_disconnect(self, session_id, connection_id):
        """
    ...
    async def dial(self, session_id, token, sip_uri, options=None):
        """
    ...
    async def set_stream_class_lists(self, session_id, payload):
        """
    ...
    async def start_broadcast(
        ...
        if "hls" in options["outputs"]:
            if (
                ...
                if (
                        options["outputs"]["hls"]["lowLatency"] is True
                        and options["outputs"]["hls"]["dvr"] is True
                ):
        ...
        if "maxBitrate" in options:
            if (
                    type(options["maxBitrate"]) is not int
    ...
    async def stop_broadcast(self, broadcast_id):
        """
    ...
    async def add_broadcast_stream(
            self,
            broadcast_id: str,
            stream_id: str,
            has_audio: bool = True,
    ...
    async def remove_broadcast_stream(
    ...
    async def get_broadcast(self, broadcast_id):
        """
    ...
    async def set_broadcast_layout(
    ...
    async def stop_render(self, render_id):
        """
    ...
    async def list_renders(self, offset=0, count=50):
        """
    ...
    async def connect_audio_to_websocket(
    ...
    async def stop_captions(self, captions_id: str):
        """
    ...
    @staticmethod
    def _sign_string(string, secret):
        return hmac.new(
    ...
    async def mute_all(
        ...
        try:
            ...
            response = await self.httpx_client.post(
                url, headers=self.get_headers(), json=options
            )
    ...
    async def disable_force_mute(self, session_id: str) -> httpx.Response:
        """
    ...
    async def mute_stream(self, session_id: str, stream_id: str) -> httpx.Response:
        """
        ...
        try:
            ...
            response = await self.httpx_client.post(url, headers=self.get_headers())
    ...
    async def play_dtmf(
            self, session_id: str, connection_id: str, digits: str, options: Optional[dict] = None
    ) -> httpx.Response:
        """
        ...
        :param options: Additional options to pass to the DTMF tones
        ...
        try:
            ...
            response = await self.httpx_client.post(
                url, headers=self.get_json_headers(), json=payload
            )
