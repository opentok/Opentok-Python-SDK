...
class OpenTokCallbackApiTest(unittest.TestCase):
    ...
    def verify_callback(self, callback):
        expect(callback).to(be_an(Callback))
        expect(callback).to(have_property(u('id'), self.callback_id))
        expect(callback).to(have_property(u('group'), self.group))
        expect(callback).to(have_property(u('event'), self.event))
        expect(callback).to(have_property(u('url'), self.url))
        if PY2:
    ...
    def verify_register_body(self):
        ...
        expect(body).to(have_key(u('group'), self.group))
        expect(body).to(have_key(u('event'), self.event))
        expect(body).to(have_key(u('url'), self.url))
    ...
    def verify_callback_list(self, callbacks):
        expect(callbacks).to(be_an(CallbackList))
        expect(list(callbacks.items)).to(have_length(1))
