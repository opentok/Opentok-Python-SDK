...
def participant():
    ...
    return render_template(
        "participant.html", api_key=key, session_id=session_id, token=token
    )
...
def history():
    ...
    show_next = (
        "/history?page=" + str(page + 1) if archives.count > (offset + 5) else None
    )
...
def start():
    ...
    archive = opentok.start_archive(
        session.session_id,
        name="Python Archiving Sample App",
        has_audio=has_audio,
        has_video=has_video,
